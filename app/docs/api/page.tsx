      ...
      <div className="docs-layout">
        ...
        <article className="docs-content">
          ...
          <section id="models" className="docs-section">
            ...
            <p>Programmatic publishing with a bearer API token (create one under <strong>Settings → API tokens</strong> after signing in). Send <code>Authorization: Bearer pc_…</code> plus the model payload: <code>name</code>, <code>source</code>, and optional <code>description</code>, <code>files</code>, <code>parameters</code>, <code>tags</code>, <code>license</code>, <code>visibility</code>, and <code>metrics</code> (the geometry measurements from your own compile, in the shape <code>/api/render</code> returns for <code>summary</code>). Without a <code>modelId</code> the request creates a new model owned by the token&apos;s account and returns its <code>/u/username/slug</code> URL; with <code>modelId</code> it publishes a new version of that model (owner-only) and moves the head. Requests without a valid token return <code>401</code> — anonymous publishing is retired, and serverless share links remain the no-account way to pass a model around.</p>
            <p>Revision records are immutable, shared, and addressed by unguessable content-hash IDs, so <code>GET /api/models/:id</code> and revision thumbnails remain readable by ID even when a private model points at them; model pages, profiles, and explore listings are where visibility is enforced.</p>
            <div className="endpoint-title secondary-endpoint"><span className="method get">GET</span><code>/api/models/:id</code></div>
            <p>Retrieve the source project, default customizer values, and inferred parameter schema for a hosted model. The response also carries the <code>metrics</code> the publisher measured (bounds, dimensions, volume, area, triangle count), or <code>null</code> when the publisher sent none. Publishing does not compile the model, so treat those numbers as reported rather than verified — call <code>/api/render</code> with <code>summary</code> to measure the geometry yourself.</p>
          </section>
          ...
          <section id="health" className="docs-section">
            ...
            <p>Readiness probe for deployments and container orchestrators. When a database is configured, a <code>200</code> response means Postgres is reachable and <code>503</code> means it is not. An engine-only instance without a database reports ready: rendering and parameter APIs work, hosted models and accounts do not.</p>
          </section>
