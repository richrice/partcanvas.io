...
- [x] **P2.2 [HUMAN]** Create GitHub and Google OAuth apps (callback `https://partcanvas.io/api/auth/callback/{github,google}` + localhost equivalents); set env vars locally and in Railway.
...
- [x] **P4.4** Manage: `PATCH /api/app/models/:id` (title, description, tags, license, visibility) and `DELETE` (removes model + history rows; revisions remain — they're content-addressed and may be shared with forks). Owner-only, tested.
...
~~Comments~~ (shipped 2026-07-18, post-plan adversarial-review pass), collections, follows/notifications, "makes" (print photos), markdown descriptions + print settings, model photo galleries, trending/decay ranking, related models, RSS/embeds, user search, policy pages + account deletion, server-side thumbnail re-rendering, email/password auth, object storage, admin dashboard, multi-replica rate limiting.
...
| 2026-07-18 | — | **Plan complete.** P5.4+D16 deployed (SUCCESS) and production-verified: health is database-only ready; `/` renders the gallery; `/new` serves the editor; `/explore` and `/?model=…` redirect; the imported revision resolves from Postgres alone. Note: the imported pre-account revision has no owner/model row, so the gallery is empty until models are published by signed-in users; its `/m/` permalink works forever. |
| 2026-07-18 | — | Gallery seeded for @richard (owner request): 4 built-in examples + 8 of his own designs (Tesla wheel trim, sharps container, 3 SKÅDIS parts, 2 LEGO tools, swatch holder) published through the full pipeline locally (compile→revision→thumbnail→model) and row-migrated to production — 12 models live with thumbnails. Skipped swatch_generator.scad (third-party, ModelMakerPro CC-BY). Fixed en route: ModelViewport fog/zoom limits now scale with model size (the 519 mm wheel trim was fogged invisible). |
| 2026-07-18 | — | **Post-plan adversarial-review pass** (owner flagged gallery inner-scroll + missing chat). Layout: content pages now scroll at document level (removed global+mobile `body{overflow:hidden}`; `.page-shell` min-height, sticky header, `.page-main` 1440px, no inner scrollbar). **Comments shipped end-to-end**: migration 0005 (`comments` table + `comment_count`/`view_count` on models), comments.server.ts (add/list/delete w/ counts in-tx; author or model-owner deletes), GET/POST `/api/app/models/:id/comments` + DELETE `/:commentId` (SOCIAL rate limit), drawer UI under the social bar (About: full description, tag links, published/updated dates, views, license; thread w/ composer, relative times, moderation). 6 new tests. |
| 2026-07-18 | — | Review fixes, same pass: **P0** hosted pages no longer overwrite the /new localStorage draft; mobile no longer hides social/pagination/settings buttons (blanket `.ghost-button` hide retargeted; dead hamburger + MoreHorizontal removed; social bar scrolls horizontally when narrow). **Owner UI** for PATCH/DELETE: Edit-details dialog (title/description/license/visibility/tags, prefilled) + 2-step Delete. OpenGraph/Twitter cards w/ thumbnail images on model+revision pages, metadataBase, profile OG, `app/sitemap.ts` + `robots.ts`. View counter (recordView on non-owner page render) on cards + bar. Cards gain comment/view counts + relative dates. Errors: toast has success/error kinds w/ shared timer; like/report/fork failures surfaced (incl. 429s); like double-click guard; report checks response.ok. Share links now target `/new?model=…` (payload was ignored on hosted pages). `key` on Workspace remounts per model (fork navigation showed stale state). `/m/:id` resolves historical versions via publish history ("snapshot of vN"); update-publish dialog drops the no-op Title field (title edits live in Edit details). Settings tokens: load-error state, create-error surfaced, revoke confirm, fresh-token banner only clears for its own row. Gallery: "Page N" indicator; profile empty state gains owner CTA; sign-in callback keeps query string. 158 tests green. |
